 ...
 *  Note-taking app with transliteration for Skyrise Travels.
...
(function () {
  ...
  function autoTitleFromContent(note) {
    // Auto-generate title from first non-empty line of content
  ...
  function openNote(id) {
    ...
    // When in Latin mode, show saved Vinara in preview, clear Latin input
    if (inputMode === 'latin') {
  ...
  function exportNoteAsPng() {
    ...
    // Use the offscreen capture target for clean rendering
  ...
  function setInputMode(mode) {
    ...
    // When switching to Latin mode, populate the preview from saved note content
  ...
  function setLatinSubMode(mode) {
    ...
    // Re-transliterate if there's Latin text in the editor
    updateLatinPreview();
  }
  ...
  function updateLatinPreview() {
    ...
    if (!text) {
      // Show saved note content if any
      if (activeId) {
        var note = notes.find(function(n) { return n.id === activeId; });
        if (note && note.content) {
  ...
  function updateStats() {
    ...
    if (inputMode === 'vinara') {
      ...
      // In Latin mode, count Vinara preview chars
      var preview = document.getElementById('vinaraPreviewContent');
  ...
  function wireEvents() {
    ...
    if (editor) {
      editor.addEventListener('keydown', function(e) {
        ...
        // In Vinara mode, convert uppercase vowels to lowercase vowel + extender
        if (inputMode === 'vinara') {
      ...
      // ── Vinara editor input (Vinara mode) ──
      editor.addEventListener('input', function() {
        if (!activeId || inputMode !== 'vinara') return;

        // Mobile-friendly: double-tapped vowels → long vowel (vowel + extender)
        // e.g. aa → aX, ee → iX, oo → uX, ey → eX, oa → oX
        ...
        for (var pair in doubleVowelMap) {
          if (doubleVowelMap.hasOwnProperty(pair)) {
            ...
            while (idx !== -1) {
              ...
              if (idx < cursor) cursor -= 1; // pair (2 chars) → repl (2 chars), same length
              changed = true;
    ...
    // Input mode toggle (Vinara / Latin)
    ...
    // Latin sub-mode toggle (Dhivehi / English)
    ...
    // Export Vinara preview as image
